...
class NunjucksCodeHighlight {
    ...
    parse(parser, nodes) {
        ...
        // Create a new template string from the body
        return new nodes.CallExtension(this, 'run', args, [body]);
    }

    run(context, lang, bodyCallback) {
        try {
            // Get the content from the body
            const content = bodyCallback();
            
            // If content is a function, call it
            const code = typeof content === 'function' ? content() : content;
            
            // Highlight the code
            const highlighted = this.hljs.highlight(code.toString(), {
            ...
            console.error('Error in NunjucksCodeHighlight:', error);
            return new this.nunjucks.runtime.SafeString(
